...
cur_path = os.getcwd()
Annotations = cur_path + '/' + 'Images' + '/' + 'Annotations'
os.makedirs(Annotations)
Main = cur_path + '/' + 'Images' + '/' + 'ImageSets' + '/' + 'Main'
os.makedirs(Main)
open('test.txt','w+').close()
shutil.move('test.txt',Main)
JPEGImages = cur_path + '/' + 'Images' + '/' + 'JPEGImages'
...
# source train dir
train_img_dir = "Images/New directory/images/"
train_txt_dir = "Images/New directory/labels/"
...
for file in os.listdir(train_img_dir):
    os.rename(os.path.join(train_img_dir,file),os.path.join(target_img_dir,"ICDAR2015_Test_" + os.path.basename(file)))
...
for idx in range(len(img_list)):
    ...
    xml_file.write('    <folder>Soutenance</folder>\n')
...
train_fd = open(target_set_dir + "/dataset/test.txt", 'w')
